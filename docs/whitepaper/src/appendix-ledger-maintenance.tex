...
\begin{protocol}
    \caption{$\textsf{EvaluationProcedure}(P, \text{sid}, R, sl, \textsf{buffer}, \textsf{syncBuffer}, \mathcal{C}_{\text{loc}},$
...
\begin{protocol}
    \caption{$\textsf{ConsensusLottery}(P, \text{sid}, e_n, \mathcal{C}_{\text{loc}}, K_{\text{f}}, K_{\text{g}}, \mathbf{S}_k^{\text{set}} = \{S_k^{\text{id}}\}_{k=1}^K,
    \mathbf{f}_{k, \text{cons}}^{\text{set}} = \{f^{\text{cons}}_k\}_{k=1}^K)$}
    \begin{algorithmic}[1]
