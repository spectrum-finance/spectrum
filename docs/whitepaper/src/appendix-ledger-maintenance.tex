...
\begin{protocol}
    ...
    \begin{algorithmic}[1]
        ...
        \State Set ${y^{\text{sync}} \leftarrow {\mathcal{H}(\textsf{SYNC} || r^{\text{sl}})}}$.
        ...
        \For {each $S_k^{\text{id}} \in \mathbf{S}_P^{\text{set}}$}
            ...
            \State Set ${y^{\text{lead}} \leftarrow {\mathcal{H}(\textsf{LEAD} || r^{\text{sl}})}}$.
            \State Set ${\textsf{valid\_leader} \leftarrow y^{\text{lead}} < T^{\text{lead}}}$.
            \If {$\textsf{valid\_leader}$}
                \State Set actual $\textsf{state}$ extracted from $\textsf{buffer}$.
                ...
                \If {$sl \in [R \cdot n, R \cdot n + 2 /\\ 3 \cdot R]$}
                    ...
                    \State {${\mathbf{b}^{\text{set}} \leftarrow \{b' \in \textsf{syncBuffer} | \textsf{validBeacon}(P, \text{sid}, R, l_{\text{VRF}}, K_{\text{f}}, b', \mathcal{C}_{\text{loc}}) = \textsf{true}\}}$}.
                    \For
                    {each $b$ in $\mathbf{b}^{\text{set}}$}
                        ...
                        \textsf{)}) = v^{\text{vrf}}^* \wedge b'.\textsf{get(} sl \textsf{)} = sl^*))$}
                            \State Remove $b$ from $\mathbf{b}^{\text{set}}$.
                        \EndIf
...
\begin{protocol}
    \caption{$\textsf{ConsensusLottery}(P, \text{sid}, e_n, \mathcal{C}_{\text{loc}}, K_{\text{f}}, K_{\text{g}}, \mathbf{S}_P^{\text{set}},
    \mathbf{f}_{k, \text{cons}}^{\text{set}} = \{f^{\text{cons}}_k\}_{k=1}^K)$}
    \begin{algorithmic}[1]
        \noindent
        \lstinline|// The lottery at the end of e_n selects committees for e_{n + 2}|
        \State Set $sl'= (n - 2)\cdot R$.
        \State Set $\textsf{state}_{sl'} \leftarrow \textsf{ReadState}(P, \text{sid}, sl', R, K_{\text{f}}, C_{\text{loc}})$.
        \For {each $S_k^{\text{id}} \in \mathbf{S}_P^{\text{set}}$}
            \State Set ${(r^{\text{e}}, \pi^{\text{e}})} \leftarrow \mathcal{F}_{\text{VRF}}\textsf{.eval(}\mathcal{H}(\eta_n || e_n) \textsf{)}$.
            \State Parse $k$-th committee stakeholders distribution $S_k^{n - 2}$ from the $\textsf{state}_{sl'}$.
            ...
            {$\textsf{is\_member} = \textsf{true}$}
                ...
                $\textsf{ConsLotteryResTx(}v^{\text{vrf}}, e_n, S_k^{\text{id}}, \pi^{\text{e}}\textsf{)}$.
            \EndIf
        \EndFor
        \lstinline|// Commit to participate in the e_{n + 3} consensus lottery:|
        \State Update your verification information broadcasting the $\textsf{VerificationUpdTx(}v^{\text{vrf}}, \mathbf{S}_P^{\text{set}}\textsf{)}$.
...
\begin{protocol}
    ...
    \begin{algorithmic}[1]
        ...
        \State Call $\textsf{ProcessBeacons}(P, \text{sid}, R, \mathbf{b}^{\text{set}}, \mathcal{C})$.
        ...
        \EndFor
        \State Set $\mathcal{C}_{\text{loc}} \leftarrow \textsf{SelectChain}(P, \text{sid}, \mathcal{C}_{\text{loc}}, R, K_{\text{f}}, K_{\text{g}}, \mathcal{N}_0, \{S_k^{\text{id}}\}_{k=1}^K, \{f^{\text{lead}}_k\}_{k=1}^K,$
        \par\hskip\algorithmicindent
