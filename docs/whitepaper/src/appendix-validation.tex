...
\begin{protocol}
    ...
    \mathbf{S}^{\text{id, set}} = \{S_k^{\text{id}}\}_{k=1}^K, \mathbf{f}_{\text{lead}}^{\text{set}} =  \{f_k^{\text{lead}}\}_{k=1}^K,$
        $\mathbf{f}_{\text{cons}}^{\text{set}} = \{f_k^{\text{cons}}\}_{k=1}^K)$}
    \begin{algorithmic}[1]
        \noindent
        \lstinline|// All indexes except the epoch index are omitted.|
        \State Parse $B$ as $(h, sl, \textsf{state}, v^{\text{vrf}}, S_k^{\text{id}}, \pi^{\text{sl}}, \sigma_{\text{KES}})$.
        ...
        $S_k^{\text{id}}, f^{\text{lead}}_k, f^{\text{cons}}_k)$
        \par\hskip
        \State Set $(e_n, V_{\text{ver}}, \eta_{n}, \eta_{n - 2}, T^{\text{cons}}, T^{\text{lead}}, T^{\text{sync}}) = \textsf{preparation\_out}$.

        \noindent
        \lstinline|// Check consensus membership:|
        \State Set $ sl^{\text{ver}} = (n - 4)\cdot R$.
        \State Set $\textsf{state}_{(n - 4)\cdot R} \leftarrow \textsf{ReadState}(P, \text{sid}, sl^{\text{ver}}, R, K_{\text{f}}, C_{\text{loc}})$.
        \State Parse verified participants table $V_{\text{ver}}$ from $\textsf{state}_{(n - 4)\cdot R}$.
        \State Parse $\mathbf{S}_{P'}^{\text{id, set}}$ related to the given $v^{\text{vrf}}$ from $V_{\text{ver}}$.
        \noindent
        ...
        \State Set $\textsf{valid\_committee} \leftarrow (S_k^{\text{id}} \in \mathbf{S}_{P'}^{\text{id, set}}) \wedge (\mathbf{S}_{P'}^{\text{id, set}} \subset \mathbf{S}^{\text{id, set}})$.
        ...
        \State Set ${y^{\text{cons}} \leftarrow {\mathcal{H}(\textsf{CONS} || r^{\text{e}})}}$.
        ...
        \State Set $\pi^{\text{sl}}_h \leftarrow \mathcal{H}(\pi^{\text{sl}}_h)$.
        \State Set $\textsf{valid\_signature} \leftarrow \mathcal{F}_{\text{KES}}\textsf{.verify(}\
        \mathcal{H}(h || \textsf{s\_hash} || sl || \pi^{\text{sl}}_h) || \sigma_{\text{KES}} || v^{\text{kes}} \textsf{)}$.

        \noindent
        \lstinline|// Check synchronization beacons:|
        \State Parse $b^{\text{set}}$ from $\textsf{state}$.
        \If{${\exists b^{\text{sync}} \in b^{\text{set}}: sl > (e_n - 1) \cdot R + 2 \cdot R \// 3}$}
...
\begin{protocol}
    \caption{$\textsf{ValidBeacon}(P, \text{sid}, R, b^{\text{sync}}, \mathcal{C})$}
    \begin{algorithmic}[1]
