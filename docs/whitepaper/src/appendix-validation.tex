...
\begin{protocol}
    \caption{$\textsf{IsValidChain}(P, \text{sid}, \mathcal{C}, R, \{S_k^{\text{id}}\}_{k=1}^K, \{f^{\text{lead}}_k\}_{k=1}^K, \{f^{\text{cons}}_k\}_{k=1}^K)$}
    \begin{algorithmic}[1]
        ...
        \For {each $e_j \in \mathcal{C}$}
            ...
            \For {each block $B \in \mathcal{C}\ | \ B\textsf{.get(}sl\textsf{)} \in e_j$}
                ...
                \State Set $\textsf{valid\_block} \leftarrow \textsf{IsValidBlock}(P, \text{sid}, R, \mathcal{C}_{\text{loc}}, B, \{S_k^{\text{id}}\}_{k=1}^K,$
                \par\hskip\algorithmicindent
