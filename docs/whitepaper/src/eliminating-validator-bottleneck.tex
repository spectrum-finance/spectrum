...
functionality unit $\mathcal{F}^k_{\text{ConnSys}}$ relevant to a specific connected system $S_k$ are selected via the consensus group lottery.
...
the lottery process does not stop, but this time, in order to initialize\
the notarization process, the leader of every committee should be determined.

The leader lottery flow for every separate $V_n^k$ is as follows:
\begin{enumerate}
    \item Before the beginning of the epoch $e_n$ every consensus group member $PK_i \in V_n^k$ requests a\
    new epoch seed $\eta_n$ from the ${\mathcal{F}}_{\text{LB}}$.
    \item Every committee member calculates leader lottery threshold value $T_n^{\text{lead}}$.
    The parameter $f$ that regulates the probability of winning is different from the one used in the consensus\
    group lottery.
    Here, it is the pre-defined value that determines how many slots will have at least one selected leader,\
    ...
    with input $x = \eta_n || sl_j $ and calculates\
    the associated random number $y$ from the received proof $\pi^{\text{sl}}$, i.e.\
    ${y = \mathcal{H}(\pi^{\text{sl}*}||\textsf{LEAD})}$, where $\pi^{\text{sl}*}$ is a random number, extracted\
    from the proof and $\textsf{LEAD}$ is a pre-defined constant.
    \item To reveal the result of the leader group lottery $PK_i$ compares $y$ with $T_n^{\text{lead}}$ and\
    if ${y < T_n^{\text{lead}}}$ then the participant is a leader
    \item Finally, $PK_i$ initiates a notarization round for $sl_j$ slot with associated\
    proofs $(sl_j, v^{\text{vrf}}_i, \pi^{\text{sl}})$ in his message.
\end{enumerate}
...
Each committee $V_n^k$ forms the notarized batches of events and adds them into their local ledgers $L^{\text{loc}, k}$.
...
in the previous epoch, i.e. ${s_i = A_m \cdot s_i^{real}}$, where $A_m$ is an authority multiplier.
...
then his actual stake ${s_i^{real}}$ is multiplied by ${A_m = 1}$.
...
party by a bounded number of blocks (equal to a security parameter $U_f$)\
...
${\{(d_k, U_k)\}_{k=1}^{K}}$, where $d_k$ is a block duration in the $S_k$ and $r_k$ is the default\
