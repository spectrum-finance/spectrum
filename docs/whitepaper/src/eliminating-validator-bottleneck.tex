...
\begin{enumerate}
    ...
    with input $x_{i, j}^{\text{lead}} = \eta_n || sl_j $ and calculates\
    the associated random number $y_{i, j}^{\text{lead}}$ from the received proof $\pi_{i, j}^{\text{sl}}$,\
    i.e. ${y_{i, j}^{\text{lead}} = \mathcal{H}(r_{i, j}^{\text{sl}}||\textsf{LEAD})}$,\
    where $r_{i, j}^{\text{sl}}$ is\
    a random number extracted from the proof and $\textsf{LEAD}$ is an arbitrary pre-defined constant.
    \item To reveal the result of the leader lottery $PK_i$ compares value $y_{i, j}^{\text{lead}}$\
    with the threshold $T_{i, n}^{\text{lead}, k}$ and\
    if ${y_{i, j}^{\text{lead}} < T_{i, n}^{\text{lead}, k}}$ then the participant is a $j$-th slot leader.\
