...
pub fn vrf_prove<const N: usize, H, Hs, TCurve: CurveArithmetic + PointCompression>(
    sk: SecretKey<TCurve>,
    message_hash: Digest<N, H>,
        ...
        Hs: Default, Hs: FixedOutput, Hs: HashMarker, Hs: Update
{
    ...
    let message_point: ProjectivePoint<TCurve> = hash_to_projective_point::<N, H, TCurve>(&message_hash);
    ...
    let c_scalar: Scalar<TCurve> = ScalarPrimitive::<TCurve>::from_bytes(GenericArray::from_slice(&c))
        .unwrap()
        .into();
...
pub fn vrf_verify<const N: usize, H, Hs, TCurve: CurveArithmetic + PointCompression>
(
    pk: PublicKey<TCurve>,
    message_hash: Digest<N, H>,
