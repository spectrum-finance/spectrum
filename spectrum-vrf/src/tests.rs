mod tests {
    ...
    fn lottery_fraction_count_test() {
        ...
        assert!(
            (wins_0 as f32 / n_iters as f32
                - selection_fraction_num as f32 / selection_fraction_denom as f32)
                .abs()
                <= 0.05
        );
        assert!(
            (wins_1 as f32 / n_iters as f32
                - selection_fraction_num as f32 / selection_fraction_denom as f32)
                .abs()
                <= 0.05
        );
        assert!(twice_win < selection_fraction_num * n_iters / 2)
    }
}
