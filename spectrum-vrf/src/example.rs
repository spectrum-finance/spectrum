mod tests {
    ...
    fn example() {
        ...
        let r_leadership =
            proof_to_random_number::<Sha256, Secp256k1>(&proof, constant_lead.as_bytes().to_vec(), vrf_range);
        if r_leadership < thr_lead {
            let r_rnd = proof_to_random_number::<Sha256, Secp256k1>(
                &proof,
                constant_rnd.as_bytes().to_vec(),
                vrf_range,
            );
        ...
        let r_sync =
            proof_to_random_number::<Sha256, Secp256k1>(&proof, constant_sync.as_bytes().to_vec(), vrf_range);
        ...
        let valid_proof = vrf_verify::<Sha256, Secp256k1>(vrf_pk, m_hash.clone(), proof).unwrap();
        assert!(valid_proof);
    }
}
