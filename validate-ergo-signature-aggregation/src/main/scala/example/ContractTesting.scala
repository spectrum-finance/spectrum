...
object ContractTesting extends IOApp {
  ...
  val service = HttpRoutes.of[IO] {
    case req @ PUT -> Root / "validate" =>
      req.as[SignatureValidationInput].flatMap { input =>
        val response = ergoClient.execute(ctx => {
          validateSignatures(ctx, input)
        })
        println(response.asJson)
        Ok(response.asJson)
      }
    case req @ PUT -> Root / "validateVault" =>
  ...
  def validateSignatures(
    ...
    val combined = for {
      address <- ErgoAddressEncoder.Mainnet.fromString(input.contract)
      exclusionSetBytes <- Base16.decode(input.exclusionSet)
      aggregateResponseBytes <- Base16.decode(input.aggregateResponse)
      aggregateCommitmentBytes <- Base16.decode(input.aggregateCommitment)
      generatorBytes <- Base16.decode(input.generator)
      identityBytes <- Base16.decode(input.identity)
      mdBytes <- Base16.decode(input.md)
      committeeBytes <- Base16.decode(input.committee)
      thresholdBytes <- Base16.decode(input.threshold)
      hashBytes <- Base16.decode(input.hashBytes)
    ...
    combined match {
      case Success(
        ...
        val eSet: ErgoValue[Coll[ExclusionSetType]] =
          ErgoValue.of(mapped, exclusionSetType)
        ...
        val dataInputs = committeeArray
          ...
          .zipWithIndex
          .map(tup => // (elements: Array[GroupElement], ix: Int)) =>
            {
              val elements = tup._1
              val ix = tup._2
              if (ix == 0) {
